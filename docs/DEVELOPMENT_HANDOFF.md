...
| 3 게임 상태·타이머 | 미구현 | 관리자 인증 방식 확정, 상태 전이, 서버 시간, 거래 마감 |
| 4 DB·거래 | 사용자·세션 DB만 선행 | Game/Company/Wallet/Portfolio/Transaction, 매수·매도·동시성 |
...
| 인증·사용자 상태 복구·게임 진행 | 후속 기능 | 연결 테스트 완료와 구분 |
...
- [ ] 관리자 버튼에서 `game:start`를 보내고 모든 화면이 한 번씩 반응하는지 확인한다.
