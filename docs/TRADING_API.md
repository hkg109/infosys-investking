...
단일 활성 게임은 고정 내부 ID로 저장합니다. 상태·현재 라운드·마감 시각·일시정지 잔여 시간을 복원하며, 상태 이벤트 저장 순서는 서버 내부 Promise queue로 보장합니다. 5단계 사건·주가 변경과 6단계 순위 확정은 이 스키마와 거래 원장 위에서 이어서 구현합니다.
