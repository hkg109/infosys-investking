...
export default function IntelligencePanel({ userId, game, revision, stale, onPurchased }) {
  ...
  return <Panel title="정보 상점·보관함">
    <p>보유 현금으로 시장 정보를 구매하세요. 구매한 본문은 본인만 볼 수 있습니다.</p>
    ...
    {current.data ? <>
      <p className="mission-points">보유 현금 <strong>{current.data.cash.toLocaleString('ko-KR')}원</strong></p>
      ...
      {review && <section className="end-confirmation" aria-label="정보 구매 확인"><h3>정보 구매 확인</h3><p>{review.title} · {review.price.toLocaleString('ko-KR')}원을 사용합니다. 구매한 정보는 보관함에 남습니다.</p>
        ...
        <button type="button" className="primary-button" disabled={!canBuy(current.data, current.data.items.find(i => i.clueId === review.clueId), options) || current.data.items.find(i => i.clueId === review.clueId)?.price !== review.price} onClick={purchase}>현금으로 구매 확정</button>
      </section>}
...
export function IntelligenceLibrary({ purchases }) {
  return <section aria-label="내 정보 보관함"><h3>내 정보 보관함</h3>{purchases.length ? <ul className="mission-list">{purchases.map(item => <li key={item.clueId}><h4>{item.title}</h4><p className="intelligence-content">{item.content}</p><p>{item.paidCash.toLocaleString('ko-KR')}원 사용 · <time dateTime={item.purchasedAt}>{new Date(item.purchasedAt).toLocaleString('ko-KR')}</time></p></li>)}</ul> : <p>아직 구매한 정보가 없습니다.</p>}</section>
}
