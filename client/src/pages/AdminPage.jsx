...
function AdminWorkspace({ adminPassword, lock }) {
  ...
  return (
    <PageLayout title="관리자 대시보드" subtitle="게임 진행 상황을 확인하고 관리합니다." actions={<button className="header-button" type="button" disabled={busy} onClick={lock}>관리자 잠금</button>}>
      ...
      <CompanyManager key={`companies-${companyVersion}`} password={adminPassword} game={gameState.game} stale={gameState.loading || Boolean(gameState.error) || gameState.pending || eventsBusy || resetBusy || missionsBusy || intelligenceBusy} onBusy={setCompaniesBusy} onChanged={() => setEventVersion(n => n + 1)} />
      <EventManager key={`events-${eventVersion}`} password={adminPassword} game={gameState.game} stale={gameState.loading || Boolean(gameState.error) || gameState.pending || companiesBusy || resetBusy || missionsBusy || intelligenceBusy} onBusy={setEventsBusy} />
      <MissionManager key={`missions-${missionVersion}`} password={adminPassword} game={gameState.game} stale={gameState.loading || Boolean(gameState.error) || gameState.pending || eventsBusy || companiesBusy || resetBusy || intelligenceBusy} onBusy={setMissionsBusy} />
      {intelligenceEnabled && <IntelligenceManager key={`intelligence-${intelligenceVersion}`} password={adminPassword} game={gameState.game} stale={gameState.loading || Boolean(gameState.error) || gameState.pending || eventsBusy || companiesBusy || resetBusy || missionsBusy} onBusy={setIntelligenceBusy} />}
