...
export function RankingResults({ data, finished }) {
  ...
  return <div className="ranking-results">
    <p>{r.final ? '게임이 종료되어 참가자 이름과 종료 시점의 확정 순위를 공개합니다.' : awaitingFinal ? '최종 결과를 확정하고 있습니다. 아래는 확정 전 익명 순위입니다.' : '현재 총자산 기준 익명 순위입니다. 거래와 주가 변화에 따라 갱신됩니다.'}</p>
    ...
    <p className="trading-help">{r.final ? '게임 종료로 참가자 이름이 공개되었습니다.' : '다른 참가자의 이름은 공개하지 않습니다. 본인 행은 ‘나’로 표시합니다.'}</p>
    <AnonymousRankingList people={r.top3} label="상위 순위" final={r.final} />
    <details className="ranking-all"><summary>전체 순위 보기 ({r.totalParticipants}명)</summary><AnonymousRankingList people={r.rankings} label="전체 순위" final={r.final} /></details>
...
export function AnonymousRankingList({ people, label, final = false }) {
  ...
  return <ol className="ranking-list" aria-label={label}>{people.map((person, index) =>
    <li key={index} className={person.isMe ? 'ranking-self' : undefined}>
      ...
      <span className="ranking-name">{final ? <strong>{person.nickname}{person.isMe ? ' (나)' : ''}</strong> : person.isMe ? <strong>나</strong> : null}</span>
