...
export default function ResetPanel({ game, canControl, pending, control, refresh, onBusy, onReset }) {
  ...
  return <Panel title="게임 초기화 · 새 게임">
    ...
    {stage === 'working' || stage === 'checking' ? <p role="status">서버에서 결과를 확인하고 있습니다...</p> : stage === 'unknown' ? <button className="secondary-button" type="button" onClick={reconcile}>초기화 결과 확인</button> : stage !== 'confirm' && <>
    ...
    <ConfirmDialog open={stage === 'confirm'} title="게임 기록 초기화" onCancel={() => setStage('idle')} onConfirm={reset} busy={stage === 'working' || pending} confirmDisabled={!canConfirmReset({ status: game?.status, canControl, pending, acknowledged, confirmation })} confirmLabel="기록 삭제 및 초기화" danger>
      <p>참가자·세션·지갑·보유 주식·주문·거래·순위·사건 배정이 삭제되며 되돌릴 수 없습니다.</p>
      ...
      <input id="reset-confirm-text" data-autofocus autoComplete="off" value={confirmation} onChange={e => setConfirmation(e.target.value)} />
    </ConfirmDialog>
  </Panel>
}
